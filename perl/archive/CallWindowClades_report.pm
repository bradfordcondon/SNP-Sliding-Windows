...
print Dumper(\%inverseCladeTracker);
####
...
TrackBestIsolate(\%allWindowsByClade);
...
my $nucsAlignedforWindow;
		...
		$finalAlt{$thisScaffold}{$start}{$backupTaxon}{"count"}	= $count;	
		#$finalAlt{$thisScaffold}{$start}{$thisTaxon}{"end"}	= $end;	
		$finalAlt{$thisScaffold}{$start}{$backupTaxon}{"adjustedCount"}	= $adjustedSNP;	
		
		#look up Clade
		my $thisClade = $cladetracker{$backupTaxon};


		$final{$thisScaffold}{$start}{$thisClade}{$backupTaxon} = $adjustedSNP; 
...
sub TrackBestIsolate {

#$final{$thisScaffold}{$Start}{$thisClade}{$backupTaxon} = $adjustedSNP; 
my %inputHash = %{shift()};
#my %outputList;
open (my $oh, '>', "ReportOut.txt") or die "could not open $outfile $!";	


foreach my $thisScaffold (keys %inputHash){
	foreach my $start (keys $inputHash{$thisScaffold}){
		foreach my $clade (keys $inputHash{$thisScaffold}{$start}){

   	 	my ($key, @keys) = keys   $inputHash{$thisScaffold}{$start}{$clade};
   		my ($big, @vals) = values $inputHash{$thisScaffold}{$start}{$clade};
		    ...
		    print $oh "$thisScaffold\t$start\t$clade\t$big\t$key\n";
		}
